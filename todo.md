...
| **4.9** | **canonical line clustering + local_thickness** | ⚠️ **4.9.2 / 4.9.3 / 4.9.4 / 4.9.6 完成**（4.9.5 是純驗證任務、未跑）| [canonical_line.py](canonical_line.py) — `canonicalize_offsets()` per (type, axis) bucket、length-weighted median、adaptive tol `clamp(0.25 × median_local_thickness, 2, 6)`。插在 `manhattan_force_axis` 之後 / `manhattan_intersection_snap` 之前。4.9.3：`local_thickness` 透過 `attach_thickness=True` 持久化到 segment 上、pipeline 結尾 strip。4.9.4：`manhattan_intersection_snap` / `manhattan_t_project` / `fuse_close_endpoints` 改為 per-segment thickness-aware tol（min-of-pair 嚴格策略；trunk-thickness for T-project）。4.9.6：proximal_bridge stroke_width compatibility gate。**4.9.4 帶來 source/sg2 同時 FAIL**（wall normal IOU drop ~3%、sg2 door normal IOU drop 5%）但視覺 overlay 顯示位置幾乎完全重合——主要是 0.1-1.5 px sub-pixel canonical 收斂差異 + 消掉 1 個 baseline 裡的 duplicate wall + 多了 1 個左邊小 horizontal wall（sg2）。待 user 視覺確認後決定是否 update baseline。**仍待做**：4.9.5（驗證 `snap_endpoints` / `snap_colinear_coords` 是否已經是 NO-OP；若是、刪除）|
