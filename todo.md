...
| **16** | **case-specific merge_collin_2 移除** | ✅ **完成**（commit 38b38e9）| 砍中段第二個 collinear merge call（純 case-specific patch）。ablation 顯示 source NO-OP、sg2 IOU 0.97 dN=0、Gemini IOU 0.95 dN+1；後續 final merge 仍會處理新的合併機會。|
| **17** | **audit chain → trunk_split topology completion** | ✅ **完成**（2026-05-13）| **本步從診斷到修法都是 audit chain 驅動**。Step 14 audit_view 新增 `chain` subcommand（[audit_view.py](audit_view.py)）：對每個 final free endpoint 反查附近被 reject 的 candidate、列主導罰 term；source 12 free 中 8 個 isolated（無附近 reject）、sg2 24 free 中 22 個 isolated。第二輪用 geometry 分類 isolated endpoint：**全部 `d=0.00`** 落在另一 segment 的 body interior 上。診斷：snap-style candidate（t_project / t_snap）對「已在 line 上」的 endpoint 不會 emit mutation→ 整個 pipeline 沒有任何 pass 會把 host trunk 拆成兩段。新 generator [`trunk_split_candidates`](generators.py)：對 degree-1 endpoint 在 perpendicular trunk body interior（perp_dist=0 + strictly interior + ≥1 px margin）emit `remove trunk + add 2 sub-trunks`，沿用 trunk type，鋪在 `_accept_trunk_split_candidates` fixed-point wrapper 裡（多 endpoint 共享同 trunk 時迭代生）。插在 final merge 之後 / stats 之前（最後一個 pass）。junction_aware merge 不會把 split 後的 sub-trunks 合回——shared interior endpoint 加上原 free seg = degree 3、external > 0 拒絕合併。**結果**：渲染 bit-identical（IOU 1.0000 全層、total length 不變）；segs source 102→114（+12 splits）、sg2 124→148（+24 splits）；free source 12→0、sg2 24→0。Baseline user-approved 更新（visual overlay 全黑、無紅綠像素）。**戰略意義**：把「diagnose → 補生成器」變成可重複流程；reviewer A 提的 bundle scoring 變成適用「有 nearby reject 但 score 擋」的小眾、剩下大宗其實是 generator 覆蓋空缺；audit chain 是這條判別線的工具。|
...
**新模組**（todo 沒列、實際產生）：
- [scoring.py](scoring.py) — score 函數（step 11 加 wall_mask 給 junction_count）
