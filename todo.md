...
| 順序 | 選項 | 評估 |
|---|---|---|
| **1** | **step 6 — Merge family unification**（進行中）| 把 `merge_collinear ×2`、`cluster_parallel_duplicates`、`manhattan_ultimate_merge` 統一成一個 `merge_candidates` generator。**Phase 1 完成（commit 005f780）**：final `manhattan_ultimate_merge` → `_accept_merge_candidates(perp_tol=0, gap_tol=0, junction_aware=True)`，PASS bit-identical（順便修了 legacy 兩個 bug：exact-duplicate dedup + body-body crossing 拒絕）。**Phase 2 暫緩**：early-pipeline 兩個 `merge_collinear` 在 canonical_line **之前**跑、score 信號弱、pair-based 跟 legacy graph-component 在 3+ chain merges 行為不同；嘗試後 revert。**Phase 3 完成（2026-05-12）**：`cluster_parallel_duplicates` → `_accept_parallel_merge_candidates(perp_tol, skip_score=True)`，source PASS bit-identical / sg2 視覺零差但有 1.3-1.5 px sub-pixel canonical 收斂偏移 + 2 個 baseline duplicate dedup，baseline 已 user-approved 更新。**設計關鍵**：`skip_score=True` 是因為 score 的 `junction` term 對「厚牆雙脊線合併」會誤判（兩條 ridge 各自接在 T-junction 上、合併會減少 junction 計數、但真實 T-junction 還在）；generator geometric gates 已夠嚴格。Phase 4 接下來：合併 phase 1 + phase 3 call sites（兩個 candidate-based merge call → 1 個）。|
