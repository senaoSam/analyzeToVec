...
| 順序 | 選項 | 評估 |
|---|---|---|
| **1** | **step 6 — Merge family unification**（進行中）| 把 `merge_collinear ×2`、`cluster_parallel_duplicates`、`manhattan_ultimate_merge` 統一成一個 `merge_candidates` generator。**Phase 1 完成（2026-05-12, commit 005f780）**：final `manhattan_ultimate_merge` → `_accept_merge_candidates(perp_tol=0, gap_tol=0, junction_aware=True)`，PASS bit-identical（順便修了 legacy 兩個 bug：exact-duplicate dedup + body-body crossing 拒絕）。**Phase 2 試做後 revert**：early-pipeline 兩個 `merge_collinear` 在 canonical_line **之前**跑，輸入是 raw skeleton 座標、不穩定；嘗試 cluster-based generator 出現 source 102→107、wall IOU drop 11% 等問題，根因是 cluster candidate all-or-nothing 跟 score 函數在 early pipeline 信號弱（canonical 未到位）互相干擾。**Phase 2 暫緩**——建議先跳到 phase 3（cluster_parallel_duplicates 跑在 canonical_line 之後、座標穩定，難度低很多）。Phase 4 可能改成「Phase 1 + Phase 3 兩個 candidate-based call 合併成 1 個 call site」（不含 phase 2）。|
